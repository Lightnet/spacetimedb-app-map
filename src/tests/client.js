...
function onInsert_Entity(ctx, row){
  ...
  MappingConfig.entities.push(row);
}

function onUpdate_Entity(ctx,oldRow, newRow){
  MappingConfig.entities=MappingConfig.entities.filter(r=>r.entityId!=row.entityId);
  MappingConfig.entities.push(row);
}
...
function onInsert_Transform3D(ctx, row){
  ...
  MappingConfig.transform3d.push(row);
}
function onUpdate_Transform3D(ctx,oldRow, newRow){
  MappingConfig.transform3d=MappingConfig.transform3d.filter(r=>r.entityId!=newRow.entityId);
  MappingConfig.transform3d.push(newRow);
  update_model_transform3d(newRow)
}
function onDelete_Transform3D(ctx,row){
  MappingConfig.transform3d=MappingConfig.transform3d.filter(r=>r.entityId!=row.entityId);
}
...
function update_pane_marker(){
  ...
  MappingConfig.markerRows.forEach((entity, index) => {
    ...
    folder.addBinding(entity, 'entityId', { label: 'Name' });
    // folder.addBinding(entity, 'position', {  });
    folder.addButton({title:'Select'}).on('click',()=>{
      // rework needed here.
      const t3 = MappingConfig.transform3d.find(r=>r.entityId == entity.entityId)
      if(t3){
...
function onInsert_MapMarker(ctx, row){
  ...
  // console.log(t3);
  // console.log("len:", MappingConfig.transform3d.length);
  if(t3){
    const _marker = create_model_marker();
    // console.log(_marker)
    // console.log(t3);
    update_model_marker_transform3d(_marker, t3);
    _marker.position.add(MappingConfig.offset_marker);
    _marker.userData.row = row;
    markers.push(_marker);//interset raycast.
    scene.add(_marker);  
  }
...
function setupDBMarker(){
  conn
    ...
    .onApplied((ctx) => {
      console.log("test");
      ctx.db.mapMarkers.onInsert(onInsert_MapMarker);
      ctx.db.mapMarkers.onUpdate(onUpdate_MapMarker);
      ctx.db.mapMarkers.onDelete(onDelete_MapMarker);
    })
    .onError((ctx, error) => {
      console.error(`Subscription failed: ${error}`);
    })
...
function setupDBMapTile(){
  conn
    ...
    .onApplied((ctx) => {
      ...
      ctx.db.mapTiles.onInsert(onInsert_MapTile);
      ctx.db.mapTiles.onUpdate(onUpdate_MapTile);
      ctx.db.mapTiles.onDelete(onDelete_MapTile);
    })
    .onError((ctx, error) => {
      console.error(`Subscription failed: ${error}`);
    })
...
//-----------------------------------------------
  ...
  const debugFolder = paneEntity.addFolder({title: 'Test'});

  debugFolder.addButton({title:'entities'}).on('click',()=>{
    console.log(MappingConfig.entities);
    console.log(MappingConfig.transform3d);
    console.log(MappingConfig.markerRows);
  })
  ...
  //   readonly: true,
  // });
  debugFolder.addBinding(MappingConfig, 'selectId',{
    readonly: true,
  });
  debugFolder.addBinding(MappingConfig, 'selectType',{
    readonly: true,
  });

  debugFolder.addBinding(MappingConfig, 'dragType',{
    readonly: true,
  });

  debugFolder.addBinding(MappingConfig, 'isDrag',{
    readonly: true,
  });
  // paneEntity.addBinding(MappingConfig, 'selectObject');
  
  paneProps = debugFolder.addFolder({title: 'Props'});

  paneMarker = debugFolder.addFolder({
    title: 'Markers',
  });
  paneMarker.element.style["max-height"] = '280px';
  paneMarker.element.style["overflow-y"] = 'auto';

  paneGrid = debugFolder.addFolder({
    title: 'Grids',
  });
